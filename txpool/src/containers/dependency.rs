...
impl Dependency {
    ...
    fn check_if_coin_input_can_spend_output(
        ...
        {
            const WRONG_OWNER_ERR: Error = Error::NotInsertedInputOutputMismatch(
                std::borrow::Cow::Borrowed("Coin owner is different from expected input"),
            );
            const WRONG_AMOUNT_ERR: Error = Error::NotInsertedInputOutputMismatch(
                std::borrow::Cow::Borrowed("Coin output does not match expected input"),
            );
            const WRONG_COLOR_ERR: Error =
                Error::NotInsertedInputOutputMismatch(std::borrow::Cow::Borrowed(
                    "Coin output color does not match expected input coin color",
                ));
            const EXPECT_CONTRACT_ERR: Error = Error::NotInsertedInputOutputMismatch(
                std::borrow::Cow::Borrowed("Expected coin but output is contract"),
            );

            const FOUND_WITHDRAWAL_ERR: Error = Error::NotInsertedInputOutputMismatch(
                std::borrow::Cow::Borrowed("Expected coin but output is withdrawal"),
            );
            ...
            match output {
                Output::Coin { to, amount, color } => {
                    if to != i_owner {
                        return Err(WRONG_OWNER_ERR.into());
                    }
                    if amount != i_amount {
                        return Err(WRONG_AMOUNT_ERR.into());
                    }
                    if color != i_color {
                        return Err(WRONG_COLOR_ERR.into());
                    }
                }
                Output::Contract { .. } => return Err(EXPECT_CONTRACT_ERR.into()),
                Output::Withdrawal { .. } => {
                    return Err(FOUND_WITHDRAWAL_ERR.into());
                }
                ...
                Output::Variable { to, amount, color } => {
                    ...
                    //do nothing, everything is variable and can be only check on execution
                }
                Output::ContractCreated { .. } => return Err(EXPECT_CONTRACT_ERR.into()),
            };
        } else {
            ...
            let is_contract = match output {
                ...
                Output::Contract { input_index, .. } => {
                    if let Input::Contract { contract_id, .. } = tx.inputs()[*input_index as usize]
                    {
                        ...
                        panic!("InputIndex for contract should be allways correct");
                    }
                }
